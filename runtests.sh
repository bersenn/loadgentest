...
locust_scripting() {
  ...
  _START=`gettimestamp`
  echo "${TESTNAME}: Executing docker run -v ${TESTDIR}:/loadgentest -i loadimpact/loadgentest-locust --host=\"${TARGETPROTO}://${TARGETHOST}\" --locustfile=${CFG_D} --no-web --clients=${CONCURRENT} --hatch-rate=${CONCURRENT} --run-time=${DURATION}s ... "
  docker run -v ${TESTDIR}:/loadgentest -i loadimpact/loadgentest-locust --host="${TARGETPROTO}://${TARGETHOST}" --locustfile=${CFG_D} --no-web --clients=${CONCURRENT} --hatch-rate=${CONCURRENT} --run-time=${DURATION}s > >(tee ${RESULTS}/stdout.log) 2> >(tee ${RESULTS}/stderr.log >&2)
  _END=`gettimestamp`
  _DURATION=`echo "${_END}-${_START}" |bc |stripdecimals`
